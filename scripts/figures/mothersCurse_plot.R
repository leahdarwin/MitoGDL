...
get_var_df <- function(df) {
  df %>%
    ...
    summarise(
      var_test = list(var.test(Y_adj[Sex == "M"], Y_adj[Sex == "F"], alternative = "greater")),
      var_ci   = list(var.test(Y_adj[Sex == "M"], Y_adj[Sex == "F"])),
      .groups = "drop"
    ) %>%
    mutate(
...
mch_tab %>%
  ...
  pack_rows("Weak MCH", 1, n_weak) %>%
  pack_rows("Strong MCH", n_weak + 1, nrow(mch_tab)) %>%
