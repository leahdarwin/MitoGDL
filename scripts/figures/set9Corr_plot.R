...
# (cor.test with method="spearman" does not return conf.int)
# -------------------------------------------------------------------
spearman_ci <- function(rho, n, conf = 0.95) {
  z      <- atanh(rho)
  se_z   <- 1 / sqrt(n - 3)
  z_crit <- qnorm((1 + conf) / 2)
  tanh(c(z - z_crit * se_z, z + z_crit * se_z))
}
...
get_corr <- function(merged, trait) {
  ...
  if (trait %in% c("Weight", "Climbing")) {
    ...
    ciMF   <- spearman_ci(testMF$estimate, nrow(merged))
