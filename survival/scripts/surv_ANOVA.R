...
# -------------------------------------------------------------------------
packages <- c("dplyr", "broom", "emmeans", "knitr", "kableExtra")
...
# Load data
# -------------------------------------------------------------------------
egg_count <- read.csv("survival/survival.csv") %>% mutate(Total = F_total + M_total)

# -------------------------------------------------------------------------
# Helpers
# -------------------------------------------------------------------------

# Tidy ANOVA table (no residuals row)
tidy_aov <- function(model) {
  tidy(anova(model)) %>%
    filter(term != "Residuals") %>%
    select(term, df, sumsq, meansq, statistic, p.value) %>%
    mutate(
      across(c(sumsq, meansq, statistic), ~round(., 4)),
